...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.mlp_branch:
            ...
            x = x + self.cab(self.ln(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
        return x
...
class Backbone_EfficientVSSM(EfficientVSSM):
    def __init__(self, out_indices=(0, 1, 2, 3), norm_layer=nn.LayerNorm, **kwargs):
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.encoder = Backbone_EfficientVSSM()
        self.decoder = SegFormerHead(in_channels=[96, 192, 384, 768])
